...
function getFormattedDateDisplay(dateStr) {
    if (dateStr === realTodayStr) return "Today";
...
function renderDashboard() {
    ...
    subjects.forEach(subject => {
        ...
        card.querySelector('.weeks-completed-text').textContent = `${completedWeeks} / ${totalWeeks} weeks`;
...
function confirmAddSubject() {
    ...
    if (durationType === 'custom') {
        ...
        if (isNaN(val) || val <= 0) {
            alert('Please enter a valid positive number for weeks.');
            return;
        }
        weeksCount = val;
    }
...
function renderDailyHabits() {
    ...
    habitsSummaryText.textContent = `${completedCount} of ${habitsData.habits.length} habits completed today`;
}
...
function renderHabitTrackerGrid(habitId, habitName) {
    ...
    for (let i = 0; i < extraCells; i++) {
        ...
        const tooltip = document.createElement('div');
        tooltip.className = 'tooltip';
        tooltip.innerHTML = `${displayDate}<br/>Future`;
        cell.appendChild(tooltip);
    ...
    if (statsElem) {
        statsElem.textContent = `Started on: ${displayStartDate} • Tracking for: ${daysTracked} day${daysTracked === 1 ? '' : 's'}`;
    }
...
function renderDailyRoutine() {
    ...
    summaryDiv.innerHTML = `<span style="font-size: 14px;">Tasks completed: ${completedTasks} / ${currentTasks.length}</span>`;
